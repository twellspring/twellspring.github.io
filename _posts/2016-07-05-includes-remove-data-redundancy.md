...
I really only want to define credentials like this in one place.  My pillar design with the application name at the second pillar level allows for multiple applications on a single server, but it also means the same data is in different pillar locations.

* apps:www:envdata:web_service1_login
* apps:customer_app:envdata:web_service1_login

This makes putting this data in a separate file and using a standard SaltStack not an option as each include file has to define the full path to the keys in that file.  Removing the app name from the app pillar solves this problem, but limits me to one app per server.  I want to keep the flexibility of multiple apps on a server, so I went looking for another option.
...
An important point is the indenting in the web_service1.sls file. My experimenting with the jinja include showed that indenting the jinua include line did not mean that all of the included lines would be correctly indented. From what I remember the first line of the include was indented and the remaining lines were not.  That does not fly with yaml as indenting is important.  This is solved by not indenting the include line and putting the correct indents in the service file.  As long as the indent level is consistent across all pillar files that include web_services1.sls, this solution works.  And since I have a well formed structure for my apps pillar, the indenting is always the same.

I like the organization and clarity this provides in my pillar file system.  All external services can be defined in one folder.  And jinja includes allow me to include this data on just the applications that need it. Problem solved.  As I look through my application pillar files I find most of them now have several jinja includes for things like:
